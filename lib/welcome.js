...
function buildWelcomeCaption({ contactName, memberId, groupName, description, memberCount, photoSource }) {
    const userTag = memberId.split('@')[0];
    const descBlock = description?.trim()
        ? `\n\n📋 *Descripción del grupo:*\n${description.trim()}`
        : '\n\n📋 _Este grupo no tiene descripción._';

    const photoNote = photoSource === 'group'
        ? '\n🖼️ _Sin foto de perfil — usando foto del grupo_'
        : photoSource === 'none'
            ? '\n🖼️ _Sin foto de perfil disponible_'
            : '';
    ...
    return (
        ...
        photoNote +
        descBlock +
        `\n\n👥 *Participantes actuales:* ${memberCount}`
    );
}
