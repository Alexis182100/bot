 ...
 * Usa el mismo patrón que removeParticipants de wwebjs:
 * Collection.get(lid) || Collection.get(phone) + enforceLidAndPnRetrieval.
 */

/** ¿Este usuario es admin AHORA en este grupo? (consulta directa en Chrome) */
    ...
    try {
        return await client.pupPage.evaluate(async (gid, idList) => {
            ...
            const lookup = (key) => {
                ...
                return (
                    arr.find((p) => {
                        ...
                        return id === key || String(id).includes(String(key).split('@')[0]);
                    }) || null
                );
            };
            ...
            for (const rawId of idList) {
                ...
                let phone = null;
                try {
                    const both = await window.WWebJS.enforceLidAndPnRetrieval(rawId);
                    lid = both?.lid?._serialized || null;
                    phone = both?.phone?._serialized || null;
                } catch (e) {}

                const candidates = [rawId, lid, phone].filter(Boolean);
...
/** Nombre real del grupo desde Chrome */
...
/**
 * Veredicto final de admin: 1) consulta directa en Chrome  2) lista fresca
 */
async function resolveIsGroupAdmin(client, chat, msg, senderNumber) {
    ...
    const ids = [...idBag];
    // 1) Chequeo directo (el más fiable con LID)
    if (await isUserAdminInGroup(client, groupId, ids)) return true;

    // 2) Lista fresca + match
    const { participant } = await findParticipantFresh(client, chat, authorId, ids);
