...
function safeLocaleDateString(date, locale, options) {
    try {
        ...
        try {
            ...
            return date.toLocaleDateString(locale, optCopy);
        } catch (e2) {
...
function safeLocaleTimeString(date, locale, options) {
    try {
        ...
        try {
            const optCopy = { ...options };
            delete optCopy.timeZone;
            return date.toLocaleTimeString(locale, optCopy);
        } catch (e2) {
...
function greetingByHour() {
    try {
        const timeStr = new Date().toLocaleTimeString('en-US', { timeZone: 'America/Mexico_City', hour12: false, hour: '2-digit' });
...
function botInfoHeader(userName, style, totalCommands, isGroup, botDisplayName) {
    const now = new Date();
    const fecha = safeLocaleDateString(now, 'es-MX', { timeZone: 'America/Mexico_City', day: 'numeric', month: 'long' });
    const hora = safeLocaleTimeString(now, 'es-MX', { timeZone: 'America/Mexico_City', hour: '2-digit', minute: '2-digit' });
