...
function activateStore(groupId, groupName) {
...
function ensureProduct(storeId, productKey, platform, category) {
    if (!meta.products[storeId]) meta.products[storeId] = {};
    if (meta.products[storeId][productKey]) return meta.products[storeId][productKey];

    const isCompleta = category === 'completa';
    const price = isCompleta ? platform.priceCompleta : platform.pricePerfil;
