...
function getDefaultPrice(platform, category) {
    return category === 'completa'
        ? (platform.priceCompleta || 10)
        : (platform.pricePerfil || 10);
}

function promptSetPrice(platform, category) {
    const def = getDefaultPrice(platform, category);
    const label = category === 'completa' ? 'cuenta completa' : 'perfil';
    return (
        `💲 *PRECIO — ${platform.emoji || ''} ${platform.name}* (${label})\n\n` +
        `Precio sugerido: *$${def}*\n\n` +
        `Escribe el precio de venta (ej: *15*)\n` +
        `o *-* para usar *$${def}*\n\n` +
        `*0.* Cancelar`
    );
}

function promptCredentials(platform, category) {
    if (category === 'perfil') {
        return (
            `👤 *PERFILES — ${platform.name}*\n\n` +
            `📧 Envía *correo y contraseña*:\n` +
            `\`correo@mail.com|contraseña\`\n\n` +
            `🤖 Se crearán *${platform.profiles} perfiles* (1–${platform.profiles})\n` +
            `*0.* Cancelar`
        );
    }
    return (
        `📺 *CUENTA COMPLETA — ${platform.name}*\n\n` +
        `📧 Envía credenciales:\n` +
        `\`correo@mail.com|contraseña\`\n` +
...
function finishStockLoad(adminId, session, platform, category, creds, storeApi) {
    ...
    storeApi.ensureProduct(storeId, productKey, platform, category, session.customPrice);
    ...
    const product = storeApi.listProducts(storeId)[productKey];
    const priceLabel = product ? ` · 💲 $${product.price}` : '';

    return (
        `✅ *STOCK AGREGADO*\n\n` +
        `${platform.emoji} *${platform.name}* (${category})${priceLabel}\n` +
        `➕ Agregado: ${typeLabel}\n` +
    ...
    if (session.step === 'new_svc_emoji') {
        ...
        if (platform.completaOnly) {
            ...
            return (
                ...
                `🔑 Clave: \`${existingKey}\`\n\n` +
                promptSetPrice(platform, 'completa')
            );
        }
...
function handleWizardInput(adminId, text, storeApi) {
    ...
    if (session.step === 'pick_platform') {
        ...
        if (platform.completaOnly) {
            ...
            return promptSetPrice(platform, 'completa');
        }
    ...
    if (session.step === 'pick_type') {
        ...
        if (body === '1') {
            touchSession(adminId, { ...session, step: 'set_price', category: 'perfil' });
            return promptSetPrice(session.platform, 'perfil');
        }
        if (body === '2') {
            touchSession(adminId, { ...session, step: 'set_price', category: 'completa' });
            return promptSetPrice(session.platform, 'completa');
        }
        return '⚠️ Responde *1* (perfiles) o *2* (completa).';
    }

    if (session.step === 'set_price') {
        let price;
        if (body === '-') {
            price = getDefaultPrice(session.platform, session.category);
        } else {
            price = parseFloat(body.replace(',', '.'));
            if (isNaN(price) || price <= 0) {
                return '⚠️ Precio inválido. Ej: *15* o *-* para usar el sugerido.';
            }
        }
        touchSession(adminId, { ...session, step: 'await_credentials', customPrice: price });
        return (
            `✅ Precio: *$${price}*\n\n` +
            promptCredentials(session.platform, session.category)
        );
    }
