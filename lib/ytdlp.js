...
const POT_PROVIDER_URL = process.env.YT_DLP_POT_PROVIDER_URL || 'http://127.0.0.1:4416';
const FORCE_IPV4 = process.env.YT_DLP_FORCE_IPV4 !== 'false';
...
function buildYoutubeExtractorArgs(playerClient) {
    const bits = ['youtube:player_skip=webpage'];
    if (playerClient) bits.push(`youtube:player_client=${playerClient}`);
    if (POT_PROVIDER_URL) {
        bits.push(`youtubepot-bgutilhttp:base_url=${POT_PROVIDER_URL}`);
    }
    return bits.join(';');
}
...
async function runYtdlp(args, timeout = 180000, {
    ...
    if (youtube) {
        ...
        if (jsRuntimes) parts.push('--js-runtimes node');
        if (FORCE_IPV4) parts.push('--force-ipv4');
        parts.push(`--extractor-args "${buildYoutubeExtractorArgs(playerClient)}"`);
        // Cookies + EJS rompen la resolución de firmas (solo imágenes disponibles).
        if (useCookies) {
...
const YT_AUDIO_EXTS = ['.mp3', '.m4a', '.opus', '.webm', '.mp4'];

const YT_AUDIO_STRATEGIES = [
    // PO Token + tv/web_embedded: mejor combo en VPS con WARP
    { label: 'tv,web_embedded', playerClient: 'tv,web_embedded' },
    { label: 'android', playerClient: 'android' },
    { label: 'web', playerClient: 'web' },
    { label: 'android_vr', playerClient: 'android_vr' },
    { label: 'android,web', playerClient: 'android,web' }
];

async function tryYtdlpAudioStrategies(videoUrl) {
    let lastErr;
    for (const { label, playerClient } of YT_AUDIO_STRATEGIES) {
        try {
            return await downloadWithYtdlp(
                ...
                YT_AUDIO_EXTS,
                120000,
                { youtube: true, useCookies: false, playerClient }
            );
        } catch (e) {
            lastErr = e;
            console.warn(`yt-dlp audio (${label}):`, e.message?.slice(0, 150));
        }
    }
    throw lastErr || new Error('yt-dlp audio falló en todas las estrategias');
}

function downloadYoutubeAudio(videoUrl) {
    return enqueueDownload(async () => {
        try {
            return await tryYtdlpAudioStrategies(videoUrl);
        } catch (e) {
...
function downloadYoutubeVideo(videoUrl) {
    return enqueueDownload(async () => {
        ...
        let lastErr;
        for (const { label, playerClient } of strategies) {
            try {
                return await downloadWithYtdlp(
                    videoUrl, fmt, 'video', exts, 180000,
                    { youtube: true, useCookies: false, playerClient }
                );
            } catch (e) {
                lastErr = e;
                console.warn(`yt-dlp video (${label}):`, e.message?.slice(0, 150));
            }
        }
        throw lastErr || new Error('yt-dlp video falló');
    });
}
